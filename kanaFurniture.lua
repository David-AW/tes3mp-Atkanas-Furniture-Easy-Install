-- Script written by Atkana https://github.com/Atkana
...
Methods.TransferOwnership = function(refIndex, cell, playerCurrentName, playerToName, save)
	...
	if playerCurrentName and myMod.IsPlayerNameLoggedIn(playerCurrentName) then
		scriptLoader.GetScript("decorateHelp").SetSelectedObject(myMod.GetPlayerByName(playerCurrentName).pid, "")
	end
end
...
local function onViewOptionSelect(pid)
	...
	if getObject(choice.refIndex, cell) then
		scriptLoader.GetScript("decorateHelp").SetSelectedObject(pid, choice.refIndex)
...
local function onInventoryOptionPlace(pid)
	...
	--Set the placed item as the player's active object for scriptLoader.GetScript("decorateHelp") to use
	scriptLoader.GetScript("decorateHelp").SetSelectedObject(pid, furnRefIndex)
end
